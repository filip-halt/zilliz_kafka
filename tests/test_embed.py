...
def runner_and_sinks():
    ...
    config.MILVUS_TOKEN = ""
    setup_kafka(config)
    insert_embedder = EmbedderInsert(config)
    search_embedder = EmbedderSearch(config)

    insert_embedder.start()
    search_embedder.start()
    ...
    yield insert_embedder, search_embedder, producer, consumer_search, consumer_insert, config
    insert_embedder.stop()
    search_embedder.stop()

def test_embedder(runner_and_sinks: Tuple[EmbedderInsert, EmbedderSearch, Producer, Consumer, Consumer, Configuration]):

    ir, sr, producer, consumer_search, consumer_insert, config = runner_and_sinks
    ...
    producer.produce(
        topic=config.KAFKA_TOPICS["INSERT_EMBEDDING_TOPIC"],
    ...
    producer.produce(
        topic=config.KAFKA_TOPICS["SEARCH_EMBEDDING_TOPIC"],
